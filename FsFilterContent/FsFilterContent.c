...
FLT_PREOP_CALLBACK_STATUS
PreWrite(_Inout_ PFLT_CALLBACK_DATA Data, _In_ PCFLT_RELATED_OBJECTS FltObjects,
	_Flt_CompletionContext_Outptr_ PVOID *CompletionContext);

FLT_POSTOP_CALLBACK_STATUS
PostWrite(_Inout_ PFLT_CALLBACK_DATA Data, _In_ PCFLT_RELATED_OBJECTS FltObjects,
	_In_opt_ PVOID CompletionContext, _In_ FLT_POST_OPERATION_FLAGS Flags);
...
--*/
{
	...
	DbgPrint("FsFilterContent Entry: Entered\n");
	...
	_In_opt_ PVOID CompletionContext, _In_ FLT_POST_OPERATION_FLAGS Flags)
{
	...
	DbgPrint("It's the Encrypt file Type");
	...
	_Flt_CompletionContext_Outptr_ PVOID *CompletionContext)
{
	...
	DbgPrint("The encrypt file Type in PreWrite");
	...
	{
		buffer = MmGetSystemAddressForMdlSafe(iopb->Parameters.Write.MdlAddress, NormalPagePriority);
	}
	if (buffer == NULL)
	{
	...
	if (buffer == NULL)
	{
		DbgPrint("Can't get buffer in PreWrite");
		return retVal;
	}
